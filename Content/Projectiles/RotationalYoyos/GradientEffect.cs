        ...
        public override void OnKill(int timeLeft)
        {
            if (Projectile.owner == Main.myPlayer && setSecondStats)
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    Projectile proj = Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), Projectile.Center, vel, 326 + Main.rand.Next(1, 3), (int)(Projectile.damage * 1.2f), 1f, Projectile.owner);
                    proj.hostile = false;
                    proj.friendly = true;
                    proj.timeLeft = 300;
                }
